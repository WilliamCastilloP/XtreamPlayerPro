...
class PlaybackBuffersTest {
    ...
    @Test
    fun `vod starts quickly and keeps enough ahead to ride a bursty panel`() {
        val vod = PlaybackBuffers.vod()
        val live = PlaybackBuffers.live()
        assertThat(vod.forPlaybackMs).isAtMost(4_000)
        assertThat(vod.forPlaybackMs).isAtLeast(live.forPlaybackMs)
        assertThat(vod.minMs).isAtLeast(35_000)
        assertThat(vod.maxMs).isAtLeast(45_000)
        assertThat(vod.maxMs).isAtMost(55_000)
        assertThat(vod.maxMs).isLessThan(120_000)
        assertThat(vod.afterRebufferMs).isAtLeast(6_000)
